def find_next_empty(puzzle):
    # finds the next row, col on the puzzle that's not filled in yet --> rep with -1
...
def is_valid(puzzle, guess, row, col):
    ...
    # and then the square 
    # thisis tricky, but we want to get where the 3x3 square starts
...
def solve_sudoku(puzzle):
    ...
    for guess in range(1,10): # range(1,10) is 1,2,3,...9
        ...
        if is_valid(puzzle, guess, row, col):
            # setp 3.1: if this is valid, then place that guess on the puzzle
